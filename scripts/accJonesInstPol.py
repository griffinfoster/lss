...
for fid,fn in enumerate(acc_files):
    for sid,sb in enumerate(sbs):
        ...
        #xxIm,imMask=lss.dftImage(xxAcc[:,:,fid,sid],uvw,px,res,mask=True)
        #xyIm=lss.dftImage(xyAcc[:,:,fid,sid],uvw,px,res,mask=False)
        #yxIm=lss.dftImage(yxAcc[:,:,fid,sid],uvw,px,res,mask=False)
        #yyIm=lss.dftImage(yyAcc[:,:,fid,sid],uvw,px,res,mask=False)
